...
module.exports.run = async function({ api, event, Users }) {
    try {
        ...
        // Check if group is approved using Groups system
        let isApproved = false;
        try {
            const Groups = require('../../includes/database/groups')({ api });
            isApproved = Groups.isApproved(String(threadID));
        } catch (error) {
            // Fallback to old config system
            isApproved = config.APPROVAL && config.APPROVAL.approvedGroups && config.APPROVAL.approvedGroups.includes(String(threadID));
        }
